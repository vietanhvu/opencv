...
public class CroppingTrapezoid {
    ...
    public CroppingTrapezoid(List<Point> cropRect, Rect imageRect) {
    ...
    public int getHit(float x, float y, float hysteresis) {
        ...
        final boolean touchesTopLeft =
                calculateDistanceToPoint(mPoints[0], mPoints[1], x, y) <= hysteresis;
        if (touchesTopLeft) {
            return HighlightView.GROW_LEFT_EDGE | HighlightView.GROW_TOP_EDGE;
        }
        final boolean touchesTopRight =
                calculateDistanceToPoint(mPoints[2], mPoints[3], x, y) <= hysteresis;
        if (touchesTopRight) {
            return HighlightView.GROW_RIGHT_EDGE | HighlightView.GROW_TOP_EDGE;
        }
        final boolean touchesBottomRight =
                calculateDistanceToPoint(mPoints[4], mPoints[5], x, y) <= hysteresis;
        if (touchesBottomRight) {
            return HighlightView.GROW_RIGHT_EDGE | HighlightView.GROW_BOTTOM_EDGE;
        }
        final boolean touchesBottomLeft =
                calculateDistanceToPoint(mPoints[6], mPoints[7], x, y) <= hysteresis;
        if (touchesBottomLeft) {
            return HighlightView.GROW_LEFT_EDGE | HighlightView.GROW_BOTTOM_EDGE;
        }

        final double topDistance =
                calculateDistanceToLine(mPoints[0], mPoints[1], mPoints[2], mPoints[3], x, y);
        if (topDistance <= hysteresis) {
            retval |= HighlightView.GROW_TOP_EDGE;
        }
        final double rightDistance =
                calculateDistanceToLine(mPoints[2], mPoints[3], mPoints[4], mPoints[5], x, y);
        if (rightDistance <= hysteresis) {
            retval |= HighlightView.GROW_RIGHT_EDGE;
        }
        final double bottomDistance =
                calculateDistanceToLine(mPoints[6], mPoints[7], mPoints[4], mPoints[5], x, y);
        if (bottomDistance <= hysteresis) {
            retval |= HighlightView.GROW_BOTTOM_EDGE;
        }
        final double leftDistance =
                calculateDistanceToLine(mPoints[0], mPoints[1], mPoints[6], mPoints[7], x, y);
    ...
    private double calculateDistanceToLine(float x1, float y1, float x2, float y2, float x,
            float y) {
